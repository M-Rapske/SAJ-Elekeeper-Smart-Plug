...
def _get_scan_interval(entry: ConfigEntry) -> timedelta:
    ...
    configured_minutes = entry.options.get(CONF_SCAN_INTERVAL_MINUTES)
    if configured_minutes is None:
        legacy_seconds = entry.options.get(CONF_LEGACY_SCAN_INTERVAL)
        try:
            configured_minutes = max(1, (int(legacy_seconds) + 59) // 60)
...
class ElekeeperDataUpdateCoordinator(DataUpdateCoordinator[list[SmartPlugInfo]]):
    ...
    async def _async_fetch_data(self) -> list[SmartPlugInfo]:
        ...
        if self._org_code is None:
            login_info = await self._client.get_login_info()
            org_code_value = login_info.raw.get("orgCode")
            if isinstance(org_code_value, str):
                self._org_code = org_code_value
        return await self._async_fetch_smart_plugs(
            plant_uid,
    ...
    async def _async_fetch_smart_plugs(
        ...
        for device in devices:
            ...
            # The list is the portal's timely source for power, total energy,
            # and switch state. The detail response may be cached, so merge
            # only telemetry that is absent from the list response.
            try:
                ...
                if isinstance(detail, dict):
                    detail_fields = {
                        ...
                        if key in _DETAIL_TELEMETRY_FIELDS and value is not None
                    }
